...
import {NativeEventEmitter, NativeModules, StatusBar, Text} from 'react-native';
...
const WorkoutScreen: React.FunctionComponent = () => {
  ...
  const interval = useDerivedValue(() => {
    if (
      ...
      return drills[currentDrillIdx.value][currentNoteIdx.value][1].toString();
    else return '';
  });
  ...
  useDerivedValue(() => {
    ...
    const moveToNextNote = () => {
      if (currentNoteIdx.value < (drills[currentDrillIdx.value] || []).length) {
        currentNoteIdx.value += 1;
      }

      if (
        currentNoteIdx.value === (drills[currentDrillIdx.value] || []).length
      ) {
        moveToNextDrill();
      }
    };
    ...
    if (noteName === currentNoteName && !(octave === '0' || octave === '1')) {
      moveToNextNote();
    }
  ...
  useEffect(() => {
    ...
    const eventListener = eventEmitter.addListener(NOTE_CALCUALTED, event => {
      const [_note, _deviationCents, _amplitude] = event.note.split(';');
